...
RECORDING_TIME = 10.0 # Seconds spent in setpoint 1.

# --- Loop Timing ---
RECORDING_RATE_HZ = 1000 # Rate at which we record data.
CONTROL_LOOP_RATE_HZ =5 # Control loop frequency.
...
# --- Plotting Setup ---
max_data_points = int(RECORDING_TIME * RECORDING_RATE_HZ)
...
try:
    ...
    print(f"Sending: Set Axis {ODRIVE_AXIS_ID} to Control Mode {CONTROL_MODE_MIT}, Input Mode {INPUT_MODE_MIT}")
    msg_set_modes = odrive_msg(ODRIVE_AXIS_ID, CMD_ID_SET_CONTROLLER_MODES,
                               (CONTROL_MODE_MIT, INPUT_MODE_MIT), '<ii')
    if msg_set_modes: bus.send(msg_set_modes)
    time.sleep(0.5)

    # Set Axis State to Closed Loop Control.
    print(f"Sending: Set Axis {ODRIVE_AXIS_ID} to Closed Loop Control State ({AXIS_STATE_IDLE})")
    ...
    loop_interval = 1.0 / RECORDING_RATE_HZ
    ...
    while running:
        ...
        if current_time < RECORDING_TIME:
            ...
            # Process the latest message if we received one.
            ...
            recording_interval = RECORDING_RATE_HZ / CONTROL_LOOP_RATE_HZ
            if loop_number > recording_interval:
                loop_number = 1
            else:
                loop_number += 1
