...
try:
    ...
    print(f"Sending: Set Axis {ODRIVE_AXIS_ID} to Control Mode {CONTROL_MODE_POSITION_CONTROL}, Input Mode {INPUT_MODE_PASSTHROUGH}")
    msg_set_modes = odrive_msg(ODRIVE_AXIS_ID, CMD_ID_SET_CONTROLLER_MODES,
                               (CONTROL_MODE_POSITION_CONTROL, INPUT_MODE_PASSTHROUGH), '<ii')
    ...
    while running:
        ...
        if current_time < (SETPOINT_1_TIME + SETPOINT_2_TIME):
            ...
            elif ((second_command_sent == False) and (current_time >= SETPOINT_1_TIME)):
